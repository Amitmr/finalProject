      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            <a class="navbar-brand" href="javascript:;">עריכת כתבה</a>
          </div>
      ...
      <div class="content">
        <div class="container-fluid">
          <div>
            <?php
                ...
                if(isset($_REQUEST['aid'])){
                    $objectArticle->setId($_REQUEST['aid']);
                }
                ...
                if(isset($_REQUEST['save'])){
                  ...
                  $objectArticle->setChanged(date('Y-m-d H:i:s'));

                  if($objectArticle->update()){
                    // header("Location: ../Articles/articles page.php");
                    echo '<div class="alert alert-success">
                            <button type="button" class="close" data-dismiss="alert" aria-label="Close">
                                <i class="material-icons">close</i>
                                </button>
                                <span>הכתבה עודכנה</span>
                          </div>';
                  } else{
                ...
                $article = $objectArticle->getArticleById();
             ?>
          </div>
          ...
          <form id="article-frm" role="form" method="post" action="">
                    ...
                    <select name="writer" class="form-control">
                      ...
                      <?php
                        while($rows = $resultsSet->FETCH_ASSOC())
                        {
                          ...
                          $reporter_ID = $rows['Id'];
                          if($reporter_ID == $article['writer']){
                              echo "<option value='$reporter_ID' selected>$reporter_name</option>";
                          }
                          else{echo "<option value='$reporter_ID'>$reporter_name</option>";}
                        }
                      ?>
                    </select>
                  </div>
                    ...
                    <select name="editor" class="form-control">
                      ...
                      <?php
                        while($rows = $resultsSet->FETCH_ASSOC())
                        {
                          ...
                          $editor_ID = $rows['Id'];
                          if($editor_ID == $article['editor']){
                              echo "<option value='$editor_ID' selected>$editor_name</option>";
                          }
                          else{
              ...
              <div class="Input">
                  <input type="text" name="title" id="title" class="Input-text" placeholder="כותרת" value="<?php echo isset($article['title']) ? $article['title'] : ''; ?>">
              </div>
              <div class="Input">
                  <textarea type="text" name="subtitle" id="subtitle" class="Input-text" placeholder="כותרת משנה"><?php echo isset($article['subtitle']) ? $article['subtitle'] : ''; ?></textarea>
              </div>
              <textarea name="content" id="content"><?php echo isset($article['content']) ? $article['content'] : ''; ?></textarea>
              <input type="submit" value="עדכן כתבה" class="btn btn-success btn-block" id="save" name="save">
          </form>
        </div>
      </div>
      <footer class="footer">
